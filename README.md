...
address_naught | When there are internal code errors that result in the wrong address being written to, more often than not that write clobbers whatever value is in address naught (`0x00000000`). | While we can't stop all the bad address writes there are two things we can do.  Put a dummy variable right at the start of the script so that these bad writes don't break something else, and enable address naught write detection in crashdetect. | [Y_Less](https://github.com/Y-Less) | |
